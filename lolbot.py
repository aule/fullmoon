...
class Bot( object ):
    class BadBot( Exception ):
        pass
    linegrep = re.compile( r'(:((?P<user>\S+)!)?(?P<address>\S+)\s+)?(?P<command>\S+)\s*(?P<params>((?<=\s)[^:\s]\S*\s*)*)((?<=\s):(?P<tail>[^\r]*))?' )
    def __init__( self, nick=None, host=HOST, port=PORT, **params ):
        ...
        if DEBUG_FLAG: print "-- Sent login info (%s,%s)" % (nick, mkuser(nick))
        while not self.active:
            self()
        if DEBUG_FLAG: print "-- initialising"
        self.init(**params)
    ...
    def __call__( self ):
        ...
        for line in temp:
            if DEBUG_FLAG: print line
            msg = self.linegrep.match(line.strip())
            if msg:
                data = msg.groupdict()
                if not data['params']: data['params'] = ""
    ...
    def PING( self, command, params, tail, **spare ):
        ...
        self.s.send( "PONG :%s\r\n" % params[-1] )
        if DEBUG_FLAG: print "--PONG! (%s)" % params[-1]
    def N001( self, **spare ):
        self.active = True
