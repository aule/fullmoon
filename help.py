...
def help( topic ):
    ...
    if topic_c in Roles.roles():
        ...
        return "Role: %s. Faction: %s. Commands: %s. %s" % (role.__name__, role.faction.__name__, ', '.join(commands(role)) or "none", role.__doc__ )

    if topic_l in Faction.groups():
        return help( [f for f in Faction.factions() if getattr(Faction,f).group == topic_l ][0] )

    if topic_c in Faction.factions():
        faction = getattr( Faction, topic_c )
        return "Faction: %s (the %s). %s" % ( faction.__name__, faction.group, faction.__doc__ )

    cmds_global = dict( [ (c.name.lower(), c.cmd.__doc__ or "Command lacks help.") for c in Narrator.cmds_global ] )
    if topic_l in cmds_global:
        return "Global command: %s. %s" % (topic_l, cmds_global[topic_l])

    cmds_private = dict( [ (c.name.lower(), c.cmd.__doc__ or "Command lacks help.") for c in Narrator.cmds_private ] )
    if topic_l in cmds_private:
        return "Private command: %s. %s" % (topic_l, cmds_private[topic_l])

    cmds_public = dict( [ (c.name.lower(), c.cmd.__doc__ or "Command lacks help.") for c in Narrator.cmds_public ] )
...
@help_topic
def global_command():
    "A command that can be used publically or privately"
@help_topic
def public_command():
    "A command which must be used publically (in the channel)"
@help_topic
def private_command():
...
@help_topic
def About():
    "Fullmoon is an online game based on Tiggers version of Werewolf, a derivative of Mafia. It was coded by Aule some time around 2009."
