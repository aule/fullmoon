...
class State:
    class NoGame: pass
    class Initialising: pass
    class Night: pass
    class WitchingHour: pass
    class Stalking: pass
    class Day: pass
    class Discussion: pass
    class Voting: pass
    class Revenge: pass
...
class Narrative:
    class DeathReveal: # must end in ". "
        ...
        innocent = RandLine( "A search of their house and property doesn't find anything incriminating. It looks like they weren't a werewolf after all. ",
    ...
    class InspectorReveal:
        ...
        cupid = RandLine( "You search through %(target)s's belongings and find a bizarre bow and arrow. Weirdo - and probably also cupid.", "You check the records and observe an unusually high attendance at weddings. In fullmoonland this is enough to declare CUPID!" )
    ...
    class Victory:
        ...
        wolves = RandLine( "Wolves win! Remaining wolves: %(wolves)s", "Wolves win! Remaining wolves: %(wolves)s", "Wolves win! Remaining wolves: %(wolves)s. Help! I'm stuck in a #bots writing factory!"  )
...
class Narrator( lolbot.Bot ):
    ...
    def throttle( self, msg ):
        size = len(msg)
        self.throttle_data.append( (size, datetime.datetime.now()+datetime.timedelta(0,2*len(self.throttle_data)) ) )
        while True:
            while self.throttle_data and self.throttle_data[0][1] < datetime.datetime.now():
    ...
    def update_topic( self ):
        ...
        if self.state == State.Voting:
            self.set_topic( "Voting to lynch %s. Wolves: %d. Villagers: %s" % ( self.lynch, len(self.roles.wolves), self.format_list( self.alive ) ) )

    def format_list( self, l, just=True ):
        if len(l) == 0: return "nobody"
        if len(l) == 1: return "just "+l[0]
    ...
    def death( self, user, msg="" ):
        ...
        if "lover" in roles:
            ...
            self.death( lover, str( Narrative.Death.lover ) % {"victim":lover, "lover":user} )
    ...
    def game_begin( self ):
        ...
        for wolf in self.roles.wolves:
            self.say( "You are a werewolf. The wolves are: %s" % self.format_list( self.roles.wolves ), wolf )
        ...
        if True:
            ...
            self.say( "You appear to own some cheese. It also appears to be of absolutely no use to you. ", self.roles.cheese )
        ...
        if self.conf.bacon:
            ...
            self.say( "You have found a massive rasher of bacon in your larder. If wolves attack, you may use it to distract the wolves so that you can get away", self.roles.bacon )
        ...
        if self.conf.cupid == 2:
            ...
            self.say( "As well as your main role, you will act as Cupid. Please use '!fire $nick1 $nick2' in the next 60 seconds to fire your bow and arrow and select these players as lovers.", self.roles.cupid )
            ...
            self.say( "Your role is Cupid. Please use '!fire $nick1 $nick2' in the next 60 seconds to fire your bow and arrow and select the lovers.", self.roles.cupid )
            self.BotCmd(["YOURROLE","cupid"], self.roles.cupid)
            sleep( 1 )
        ...
        if self.conf.cupid == 3:
            ...
            sleep( 2 )
            self.say( "You are one of the Lovers. You are madly in love with %s." % self.roles.lovers[0], self.roles.lovers[1] )
            self.say( "You are one of the Lovers. You are madly in love with %s." % self.roles.lovers[1], self.roles.lovers[0] )
        ...
        if target and not recruit and self.roles.witch:
            ...
            if target == self.roles.witch:
                if "beer" in self.potions:
                    self.say( "You have a vision... the wolves are going to attack YOU! You have 2 mins to save yourself with '!beer'%s." % ("" or "cider" in self.potions and " or attack with '!cider $target'"), self.roles.witch )
                    self.BotCmd( ["WITCHSEE","yourself",True,("cider" in self.potions)] , self.roles.witch)
                elif "cider" in self.potions:
                    self.say( "You have a vision... the wolves are going to attack YOU! You have 2 mins to save yourself but, OH NOES! You already used your beer! You have one last chance to strike back with '!cider $target'.", self.roles.witch )
                    self.BotCmd( ["WITCHSEE","yourself",False,True] , self.roles.witch)
                else:
                    self.say( "You have a vision... the wolves are going to attack YOU! You have 2 mins to save yourself but, OH NOES! You already used your beer! Nothing to do but use '!sleep' and hope you don't wake up again to feel it. Sweet dreams!", self.roles.witch )
                    self.BotCmd( ["WITCHSEE","yourself",False,False] , self.roles.witch)
                self.set_timer( 90, self.game_witch_warning )
            else:
                if self.potions:
                    options = []
                    if "beer" in self.potions: options.append("save them with '!beer'")
                    if "cider" in self.potions: options.append("poison someone with '!cider $target'")
                    self.say( "You have a vision... the wolves are going to attack %s! You have 2 mins to %s." % (target, ' or '.join(options)), self.roles.witch )
                    self.BotCmd( ["WITCHSEE",str(target),("beer" in self.potions),("cider" in self.potions)] , self.roles.witch)
                    self.set_timer( 90, self.game_witch_warning )
                else:
                    self.say( "You have a vision... the wolves are going to attack %s! There is nothing you can do though as you have used all your potions." % target, self.roles.witch )
                    self.BotCmd( ["WITCHSEE",str(target),False,False] , self.roles.witch)
            ...
            if "cider" in self.potions:
                ...
                self.BotCmd( ["WITCHSEE",False,False,("cider" in self.potions)] , self.roles.witch)
                self.set_timer( 90, self.game_witch_warning )
            else:
                self.set_timer( 0, self.game_wolf_kill )
        else:
    ...
    def test_victory( self ):
        if self.alive == list(self.roles.lovers): return self.victory_lovers()
        if len(self.roles.wolves) == 0: return self.victory_innocent()
        if len(self.alive) <= len(self.roles.wolves) * 2 - ("cider" in self.potions): return self.victory_wolves()
        return False
    ...
    def cmd_stop( self, user ):
        ...
        self.say( "Game will be stopped by %s in 20 seconds. Use '!abort' to override this and continue the game." % user )
...
#                    self.sheepdata.append(Sheep3.SheepBot(botname))
                    self.sheepdata[botname] = Sheep3.SheepBot(botname, votes=Vote, mayorname=self.mayorname )
                else:
    ...
    def cmd_fire( self, user, loverA, loverB ):
        if loverA == user or loverB == user:
            ...
            self.roles.lovers = ( self.alive[ self.alive.index( loverA ) ], self.alive[ self.alive.index( loverB ) ] )
