...
import re, datetime, random, traceback
...
def cmd_requirement( state=None, allow_self=True, required_role=None, require_alive=True, require_target_alive=True, allow_abstain=False ):
    ...
    def _wrapper( f ):
        def _wrapped( self, user, *args, **kwargs ):
            ...
            if not allow_self and kwargs.get("target") == user:
                ...
                return
            if require_target_alive and kwargs.has_key("target") and kwargs.get("target") not in self.alive:
                if not ( allow_abstain and ' ' in kwargs.get("target") ): # abstain targets contain a space (eg. "don't care")
                    self.say( "They are not alive", user )
                return
            if required_role and not ( hasattr( user, "character" ) and issubclass(user.character.role, required_role) ):
                self.say( "Only players with the role of %s can do that" % required_role.__name__, user )
                return
...
class Narrator( Bot ):
    ...
    def init( self, channel, log, botInterface ):
        ...
        self.alive = []
        self.ghosts = []
        self.start_test = lambda: None # this is later replaced with a fn that may start the game if enough players
        self.help = False
        self.s.send( "JOIN %s\r\n" % self.channel )
    ...
    def update_topic( self ):
        ...
        if self.state == State.NoGame:
            self.set_topic( "Waiting list for next game: %s (%d)" % ( self.format_list( self.waiting ), len(self.waiting)) )
        ...
        if self.state in [State.Night, State.WitchingHour, State.Stalking]:
            self.set_topic( "Night time. Wolves: %d. Villagers: %s" % ( len(self.roles.wolves), self.format_list( self.alive ) ) )
            self.botSend( ( "PLAYERS", [ str(u) for u in self.alive ], ) )
            self.botSend( ( "WOLVES", len(self.roles.wolves), ) )
        if self.state in [State.Day, State.Discussion]:
            self.set_topic( "Day time. Wolves: %d. Villagers: %s" % ( len(self.roles.wolves), self.format_list( self.alive ) ) )
            self.botSend( ( "PLAYERS", [ str(u) for u in self.alive ], ) )
            self.botSend( ( "WOLVES", len(self.roles.wolves), ) )
        if self.state == State.Voting:
            self.set_topic( "Voting to lynch %s. Wolves: %d. Villagers: %s" % ( self.nominee, len(self.roles.wolves), self.format_list( self.alive ) ) )

    def format_list( self, l, just=True ):
        if len(l) == 0: return "nobody"
        if len(l) == 1: return "just "+str(l[0])
        return ', '.join([str(u) for u in l[:-1]])+" and "+str(l[-1])

    def format_conf( self ):
        yn = ["no","yes"]
        return "wolves: %d; witch: %s; inspector: %s; hunter: %s; cupid: %s; bacon: %s; wolf recruiting: %s; efficient government: %s; survival chance: %d%%" % (
            self.conf.wolves,
            yn[self.conf.witch],
    ...
    def death( self, user, msg="" ):
        ...
        msgs = [msg]
        msgs.append( getattr( Narrative.DeathReveal, user.character.role.__name__.lower() ) )
        ...
        if user.character.lover:
            ...
            self.death( user.character.lover, str( Narrative.Death.lover ) % {"victim":user.character.lover, "lover":user} )

    def hunter_death( self, callback ):
        self.state = State.Revenge
        def revenge( target ):
            self.botSend( ("DEATHREASON", target.isolate(), "HUNTER" ) )
            self.death( target, str( Narrative.Death.hunterAttack ) % {"hunter":self.roles.hunter, "victim":target} )
            self.set_timer( 0, callback )
        self.update_topic()
        self.revenge = revenge
        self.say( "You have 60 seconds to take revenge and shoot someone with your big gun.", self.roles.hunter )
    ...
    def game_begin( self ):
        ...
        if len(self.alive) < 3:
            self.say( "Not enough players: at least three players are required to avoid instant Game Over" )
            self.state = State.NoGame
            return # just give up
        needed = self.conf.wolves + self.conf.witch + self.conf.hunter + self.conf.inspector + (self.conf.cupid==1)
        if needed > len(self.alive): # we have a problem
            if self.conf.cupid == 1:
                ...
                needed -= 1
                self.say( "Not enough players: changing cupid rule to Passive Cupid instead" )
        if needed > len(self.alive):
            n = min( self.conf.wolves-1, needed - len(self.alive) )
            self.conf.wolves -= n
            needed -= n
            self.say( "Not enough players: changing number of wolves to %d" % self.conf.wolves )
        if needed > len(self.alive):
            if self.conf.hunter:
        ...
        if len(self.alive) < self.conf.wolves*2+1:
            ...
            self.say( "Too many wolves: changing number of wolves to %d" % self.conf.wolves ) 
        ...
        for player in self.characters:
            ...
            self.say( getattr( Narrative.RoleAssignment, player.role.__name__.lower() ), player )
            for role in player.passive_roles:
                self.say( getattr( Narrative.RoleAssignment, role.__name__.lower() ), player )
        ...
        self.say_list( "The wolf pack contains: %s" % self.format_list( self.roles.wolves ), self.roles.wolves )
        self.botSend( ( "YOURFACTION", [p.user.isolate() for p in self.roles.wolves] ), [p.user for p in self.roles.wolves] )
    ...
    def game_make_lover( self ): 
        if self.conf.cupid > 0: # if there should be lovers
            if not self.roles.lovers: # this could be auto cupid, or if cupid was too slow/lazy
                lovers = random.sample( self.characters, 2 )
                lovers[0].lover = lovers[1]
                lovers[1].lover = lovers[0]
                if self.conf.cupid < 3: # not auto cupid
                    self.say( "You did not choose the lovers in time, so %s and %s have been chosen automatically" % lovers, self.roles.cupid )
            self.say( "You are one of the Lovers. You are madly in love with %s." % lovers[0], lovers[1] )
            self.say( "You are one of the Lovers. You are madly in love with %s." % lovers[1], lovers[0] )
    ...
    def game_night( self ):
        ...
        self.cider = None
        if self.recruit:
            self.say_list( "You have 2 mins to make your attack. You may still recruit another Wolf with '!scratch'.", self.roles.wolves )
        else:
            self.say_list( "You have 2 mins to make your attack. Don't forget you must not attack different targets.", self.roles.wolves )
    ...
    def game_wolf_attack( self ):
        ...
        if target and target.character.has_item("bacon"):
            ...
            self.say_list( str( Narrative.Notify.Wolves.baconEscape ) % {'target':target}, self.roles.wolves )
            self.botSend( ("NWESCAPE", target.isolate(), "BACON" ), [p.user for p in self.roles.wolves] )
        ...
        if target and random.uniform(0,100) < self.conf.survival:
            ...
            self.say_list( str( Narrative.Notify.Wolves.survivalEscape ) % {'target':target}, self.roles.wolves )
            self.botSend( ("NWESCAPE", target.isolate(), "SURVIVAL" ), [p.user for p in self.roles.wolves] )
        ...
        if target and not recruit and witch:
            ...
            if target == witch:
                if witch.has_item("beer"):
                    self.say( "You have a vision... the wolves are going to attack YOU! You have 2 mins to save yourself with beer%s." % ("" or witch.has_item("cider") and " or attack with cider'"), witch )
                elif witch.has_item("cider"):
                    self.say( "You have a vision... the wolves are going to attack YOU! You have 2 mins to save yourself but, OH NOES! You already used your beer! You have one last chance to strike back with cider.", witch )
                else:
                    self.say( "You have a vision... the wolves are going to attack YOU! You have 2 mins to save yourself but, OH NOES! You already used your beer! Nothing to do but use sleep and hope you don't wake up again to feel it. Sweet dreams!", witch )
                self.set_timer( 90, self.game_witch_warning )
            else:
                if witch.has_item("beer") or witch.has_item("cider"):
                    options = []
                    if witch.has_item("beer"): options.append("save them with beer")
                    if witch.has_item("cider"): options.append("poison someone with cider")
                    self.say( "You have a vision... the wolves are going to attack %s! You have 2 mins to %s." % (target, ' or '.join(options)), witch )
                    self.set_timer( 90, self.game_witch_warning )
                else:
                    self.say( "You have a vision... the wolves are going to attack %s! There is nothing you can do though as you have used all your potions." % target, witch )
                    self.set_timer( 0, self.game_wolf_kill )
            self.botSend( ("WITCHSEE", User(target).isolate(), witch.has_item("beer"), witch.has_item("cider") ), witch.user )
        elif witch:
            self.say( "You wake up in the night, but for once there are no bad dreams. It looks like the wolves won't be killing tonight. ", self.roles.witch )
            if witch.has_item("cider"):
                self.state = State.WitchingHour
                self.say( "You still have cider you can use to poison somebody. You have 2 mins. ", self.roles.witch )
                self.set_timer( 90, self.game_witch_warning )
                self.botSend( ("WITCHSEE", None, witch.has_item("beer"), witch.has_item("cider") ), witch.user )
            else:
                self.set_timer( 0, self.game_wolf_kill )
        else:
            self.set_timer( 0, self.game_wolf_kill )
    ...
    def game_witch_attack( self ):
        if self.cider:
            if self.cider.character.role == Roles.Hunter:
                ...
                self.botSend( ("DEATHREASON", self.roles.hunter.user.isolate(), "WITCH" ), self.roles.hunter.user )
    ...
    def game_wolf_kill( self ):
        if self.wolf_attack[0] and not self.beer:
            if self.wolf_attack[0].character == Roles.Hunter and not self.wolf_attack[1]:
                self.say( str( Narrative.Notify.Hunter.wolfAttack ), self.roles.hunter )
                self.botSend( ("DEATHREASON", self.roles.hunter.user.isolate(), "WOLF" ), self.roles.hunter.user )
    ...
    def game_inspector( self ):
        ...
        if self.roles.inspector and not self.stalked:
            ...
            self.say( "You have 60 seconds left to choose who to stalk.", self.roles.inspector )
    ...
    def game_dawn( self ):
        ...
        if self.cider:
            ...
            self.death( self.cider, str( Narrative.Death.witchAttack ) % {"victim":self.cider} )
        if not self.wolf_attack[0]:
            self.say( str( Narrative.Notify.All.wolfFail ) )
        else:
            if self.wolf_attack[1]:
                self.say( str( Narrative.Notify.Target.wolfScratch ) % {'wolves': self.format_list(self.roles.wolves)}, self.wolf_attack[0] )
                self.say( str( Narrative.Notify.All.wolfScratch ) )
                self.botSend( ("NWWOLFRECRUIT", self.wolf_attack[0].isolate() ), [p.user for p in self.roles.wolves] )
                self.botSend( ("NTWOLFRECRUIT", [p.user.isolate() for p in self.roles.wolves] ), self.wolf_attack[0]  )
                self.botSend( ("WOLFRECRUIT",) )
                self.roles.wolves.append( self.wolf_attack[0] )
                self.recruit = False
            elif self.beer:
                self.say_list( str( Narrative.Notify.Wolves.beerEscape ) % {'target':self.wolf_attack[0]}, self.roles.wolves )
                self.botSend( ("NWESCAPE", self.wolf_attack[0].isolate(), "BEER" ), [p.user for p in self.roles.wolves] )
                self.botSend( ("NTESCAPE", "BEER" ), self.wolf_attack[0] )
                if self.wolf_attack[0] != self.roles.witch:
                    self.say( str( Narrative.Notify.Target.beerEscape ), self.wolf_attack[0] )
            else:
                self.botSend( ("DEATHREASON", self.wolf_attack[0].isolate(), "WOLF" ) )
                self.death( self.wolf_attack[0], str( Narrative.Death.wolfAttack ) % {"victim":self.wolf_attack[0]} )
    ...
    def game_proposals( self ):
        ...
        if len(self.alive) == len(self.roles.wolves) * 2:
            self.say( "In this situation, to avoid stalemate, if 50% of villagers use '!pass' then no lynching will take place." )
    ...
    def game_vote_warning( self ):
        ...
        self.say( "Waiting for %s to vote. You have 30 seconds left. " % self.format_list( unvoted ) ) 
    ...
    def game_count_votes( self ):
        ...
        if self.nominee.character == Roles.Hunter:
            self.botSend( ("DEATHREASON", self.roles.hunter.user.isolate(), "LYNCH" ), self.roles.hunter.user )
    ...
    def game_lynch( self ):
        ...
        self.death( self.nominee, str( Narrative.Death.lynched ) % {"victim":self.nominee} )
        self.lynchings -= 1
        if self.lynchings:
    ...
    def test_victory( self ):
        if self.characters == self.roles.lovers: return self.victory_lovers()
        if self.characters == self.roles.villagers: return self.victory_innocent()
        if self.characters == self.roles.wolves: return self.victory_wolves()
        if self.characters == self.roles.vampires: return self.victory_vampires()
        if self.characters == self.roles.zombies: return self.victory_zombies()
    ...
    def victory_innocent( self ):
        self.say( str( Narrative.Victory.innocent ) % {'villagers':self.format_list(self.alive)} )
        self.botSend( ("VICTORY", "Innocent" ) )
        self.set_timer( 10, self.game_over )
        return True

    def victory_wolves( self ):
        self.say( str( Narrative.Victory.wolves ) % {'wolves':self.format_list(self.roles.wolves)} )
        self.botSend( ("VICTORY", "Wolves" ) )
        self.set_timer( 10, self.game_over )
        return True

    def victory_vampires( self ):
        self.say( str( Narrative.Victory.vampires ) % {'vampires':self.format_list(self.roles.vampires)} )
        self.botSend( ("VICTORY", "Vampires" ) )
        self.set_timer( 10, self.game_over )
        return True

    def victory_zombies( self ):
        self.say( str( Narrative.Victory.zombies ) % {'zombies':self.format_list(self.roles.zombies)} )
    ...
    def victory_lovers( self ):
        ...
        self.say( str( Narrative.Victory.lovers ) % {'lover1':self.roles.lovers[0],'lover2':self.roles.lovers[1]} )
        self.set_timer( 10, self.game_over )
        return True
    ...
    def cmd_join( self, user ):
        ...
        if user not in self.waiting:
            ...
            self.say( "You will be included in the next game. To leave this waiting list, use '!leave'.", user )
            self.update_topic()
            self.start_test()
        else:
    ...
    def cmd_leave( self, user ):
        ...
        try:
            ...
            self.say( "You will no longer be included in the next game. To re-enter the waiting list, use '!join'.", user )
            self.update_topic()
        except ValueError:
            self.say( "You are not in the waiting list. To leave a game that is running, use '!suicide'.", user )

    def cmd_clear( self, user ):
        "Usage: '!clear'. Removes everybody from the waiting list for a game."
        self.say( "The waiting list has been cleared by %s. The waiting list contained %s. Use '!join' to enter the new waiting list." % (user, self.format_list( self.waiting ) ) )
        self.waiting = []
        self.update_topic()
    ...
    def cmd_kick( self, user, target ):
        ...
        try:
            ...
            self.say( "%s has been kicked from the waiting list by %s. To re-join the next game, use '!join'." % (target,user) )
            self.update_topic()
        except ValueError:
            self.say( "They are not in the waiting list. You may not kick someone from an active game.", user )
    ...
    def cmd_start( self, user ):
        ...
        if self.state == State.NoGame:
            ...
            self.say( "Game started by %s. Villagers are: %s. Please wait while roles are assigned." % (user, self.format_list( self.alive ) ) )
            self.game_begin()
    ...
    def cmd_stop( self, user ):
        ...
        self.say( "Game will stopped by %s in 20 seconds. Use '!abort' to override this and continue the game." % user )
    ...
    def cmd_abort( self, user ):
        ...
        try:
            if self.check_timer("stop timer"):
                ...
                self.say( "Game stop sequence aborted by %s. The game will now continue." % user )
    ...
    def cmd_conf_default( self, user ):
        self.conf = Config()
        self.say( "Configuration has been restored to default values... %s" % self.format_conf(), user )
    ...
    def cmd_conf_set( self, user, option, value ):
        ...
        if option == "wolves":      self.conf.wolves = max(1,int(value))
        elif option == "witch":     self.conf.witch = value == "yes"
        elif option == "hunter":    self.conf.hunter = value == "yes"
        elif option == "inspector": self.conf.inspector = value == "yes"
        elif option == "cupid":     self.conf.cupid = ["no","yes","passive","auto"].index(value)
        elif option == "bacon":     self.conf.bacon = value == "yes"
        elif option == "recruiting":self.conf.recruiting = value == "yes"
        elif option == "EG":        self.conf.doublelynch = value == "yes"
        elif option == "survival":  self.conf.survival = min(100,max(0,int(value)))
    ...
    def cmd_crash_test( self, user ):
        ...
        if self.check_timer():
            ...
            self.say( "Something seems to have gone wrong. State is State.%s" % self.state.__name__ )
    ...
    def cmd_propose( self, user, target ):
        ...
        if target == self.nominee and self.last:
            ...
            if delay < 15:
                self.say( "Please wait %d seconds before proposing %s again" % (15-delay,target), user )
                return
    ...
    def cmd_second( self, user, target ):
        ...
        if self.proposals.get( user, None ) == target:
            ...
            return
        self.proposer = [player for player in self.proposals.keys() if self.proposals[player] == target ][0]
        self.seconder = user
    ...
    def cmd_fire( self, user, loverA, loverB ):
        ...
        if loverA == user or loverB == user:
            ...
            lovers = ( self.characters[ self.characters.index( loverA ) ], self.characters[ self.characters.index( loverB ) ] )
            lovers[0].lover = lovers[1]
            lovers[1].lover = lovers[0]
            self.say( "Hit by your arrows, %s and %s fall madly in love." % self.roles.lovers, user )
            self.set_timer( 3, self.game_make_lover )

    @cmd_requirement( State.Night, required_role=Roles.Wolf, allow_self=False, allow_abstain=True )
        ...
        if ' ' in target:
            ...
            for wolf in self.roles.wolves:
                if wolf != user:
                    ...
                    self.botSend( ("OTHERWOLFPICKED", user.isolate(), User(target).isolate(), False ), wolf.user )
        ...
        if ' ' in target:
            ...
            for wolf in self.roles.wolves:
                if wolf != user:
                    ...
                    self.botSend( ("OTHERWOLFPICKED", user.isolate(), User(target).isolate(), True ), wolf.user )
        if len(self.votes) == len(self.roles.wolves):
            self.set_timer( 10, self.game_wolf_attack )
    ...
    def cmd_cider( self, user, target ):
        ...
        if target == self.wolf_attack[0] and not self.wolf_attack[1]:
            self.say( "The wolves are already after %s. Stop being silly." % target, user )
            return
    ...
    @cmd_requirement( [State.Night, State.WitchingHour, State.Stalking], require_alive=False )
    ...
    @cmd_requirement( [State.Night, State.WitchingHour, State.Stalking], required_role=Roles.Inspector )
    ...
    cmds_global = [
        ...
        Command( '^!conf (?P<option>witch|hunter|inspector|bacon|recruiting|EG) (?P<value>yes|no)$', cmd_conf_set ),
        Command( '^!conf (?P<option>wolves) (?P<value>\d+)$', cmd_conf_set ),
        Command( '^!conf (?P<option>cupid) (?P<value>no|yes|passive|auto)$', cmd_conf_set ),
        ...
        Command( '^!crash recovery @State\.(?P<state>\w+)$', cmd_crash_fix, name="crash recovery" ),
        Command( '^!copyover$', cmd_copyover ),
        Command( '^!reboot$', cmd_reboot ),
        Command( '^!botsay (>(?P<target>\S+) )?(?P<msg>.+)$', cmd_bot_say ),
        Command( '^!sock$', cmd_ask_sock ),
    ] 

    cmds_private = [
        Command( '^!help (?P<topic>.+)$', cmd_help ),
        Command( '^!fire (?P<loverA>\S+) (?P<loverB>\S+)$', cmd_fire ),
        Command( '^!(bite|kill|attack) (?P<target>\S+|don\'?t care)$', cmd_bite, "bite" ),
        Command( '^!(scratch|recruit) (?P<target>\S+)$', cmd_recruit, "scratch" ),
        ...
        Command( '^!bot (?P<name>\S+)$', cmd_bot_register ),
    ...
    def game_continue( self ):
        if self.state == State.NoGame:
            ...
            self.say( "Was waiting for witch to respond. Witch will be given 30 seconds to respond. " )
            return self.game_witch_warning()
        elif self.state == State.Stalking:
            self.say( "Was waiting for investigator to respond. Investigator will be given 60 seconds to respond. " )
            ...
            self.say( "Waiting for hunter to take revenge. Not sure where to restart. Trying dawn. " )
