...
@Faction.add
class Vampire:
    "Vampires attack at night independently. Each vampire may attack a different target, but does not know anything about the other vampires."
    group = "vampires"
...
class Roles:
    ...
    def roles(cls):
        return [
            ...
            if type(x) == type(cls) and x.__name__ != "Role"
        ]
...
@Roles.add
class Wolf(Role):
    "This role gives the player the opportunity to make kills each night, as part of a pack."
    faction = Faction.Werewolf


@Roles.add
class Vampire(Role):
    "This role gives the player the opportunity to kill every night, independent of other vampires"
...
class RoleManager(object):
    ...
    def __getattribute__(self, role):
        ...
        if role == "lovers":  # done a little differently, filter out to save time
            ...
            result = [
                p for p in self.narrator.characters if p.faction.group.lower() == role
            ]
            ...
            result = [
                ...
                p
                for p in self.narrator.characters
                if role in [r.__name__.lower() for r in p.passive_roles]
            ]
            result = (
                result and result[0] or None
            )  # roles and passive roles are assumed to be unique here
        else:
