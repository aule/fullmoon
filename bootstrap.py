...
import sys, datetime, traceback, socks, pickle
...
def main( nick="Mayor", channel="#fullmoon", logpath="fullmoon.log", host="localhost", port=6667, tick=0.2, socket="fullmoon.sock" ):
    ...
    while True:
        try:
            copyover()
            n = fullmoon.Narrator( nick, channel=channel, host=host, port=port, log=log, botInterface=botInterface )
            while True:
                try:
                    n()
                    sleep( tick )
